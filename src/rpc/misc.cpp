...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total nyx balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized nyx balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "nyx|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "nyx")
            + HelpExampleRpc("debug", "nyx,net")
        );
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"nyxaddress\"\n"
            "\nReturn information about the given nyx address.\n"
            "\nArguments:\n"
            "1. \"nyxaddress\"     (string, required) The nyx address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"nyxaddress\", (string) The nyx address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Nyx address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are nyx addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) nyx address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"nyxaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"nyxaddress\"  (string, required) The nyx address to use for the signature.\n"
...
UniValue getaddressmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "    \"satoshis\"  (number) The difference of nuffs\n"
...
UniValue getaddressutxos(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "    \"satoshis\"  (number) The number of nuffs of the output\n"
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"balance\"  (string) The current balance in nuffs\n"
            "  \"received\"  (string) The total number of nuffs received (including change)\n"
            "}\n"
            "\nExamples:\n"
